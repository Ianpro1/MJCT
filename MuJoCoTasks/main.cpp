...
int main() {
	...
	//initialize const for observation pointer and reward
	if (m->nq + m->nv != 10)
	{
		std::cout << "wrong size for qpos and qvel";
		return -1;
	}
	const int nq = 5;
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//rewards (positive reward = d->sensordata[2], negative reward = d->sensordata[3])
		...
		//observation (should be d->qpos + d->qvel)

		for (int i = 0; i < nq; i++)
		{
			observation[i + nq] = d->qpos[i];
